...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    class Glower {
        ...
        draw() {
            ...
            this.grad = canvas_context.createRadialGradient(this.x,this.y, 0,this.x, this.y, this.radius)
            this.grad.addColorStop(0, this.color+"FF")
            this.grad.addColorStop(1, this.color+"00")
            canvas_context.strokeStyle = 'transparent'
            canvas_context.beginPath();
            if (this.radius > 0) {
                canvas_context.arc(this.x, this.y, this.radius, 0, (Math.PI * 2), true)
...
//         console.log(gamepadAPI.axesStatus[1]*gamepadAPI.axesStatus[0]) //debugging
    ...
    class Plant{
        constructor(){
            this.flower = new Glower(Math.random()*1280, 560, Math.random()*100, getRandomColor())
            this.flower.reflect = 1
            this.stem = [this.flower]
            this.xdir  = (Math.random()-.5)*4
            this.flower.ymom =  4
            this.ydir = Math.random()*5
            for(let t = 1;t<15;t++){
                let circ = new Circle(this.flower.x + (this.flower.xmom*t), this.flower.y+(t*10), 5, "#00DD00", 0, 0)
                this.stem.push(circ)
            }
            this.rubber = []
            for(let t = 0;t<this.stem.length;t++){
                for(let k = 0;k<this.stem.length;k++){
                    if(t!=k){
                        if(!this.stem[t].touched.includes(k)){
                            if(!this.stem[k].touched.includes(t)){
                                let link = new LineOP(this.stem[t], this.stem[k])
                                if(link.hypotenuse() < 14){
                                    let spring = new SpringOP(this.stem[t], this.stem[k], link.hypotenuse(), t)
                                    this.rubber.push(spring)
                                    this.stem[t].touched.push(k)
                                    this.stem[k].touched.push(t)
                                }
                            }
                        }
                    }
        ...
        draw(){ 
            ...
            this.flower.xmom = this.xdir
            this.flower.ymom = this.ydir
            console.log(this)

            for(let t = 0;t<this.rubber.length;t++){
                this.rubber[t].balance()
            }
            this.flower.move()

            for(let t = 0;t<this.stem.length;t++){
                this.stem[t].ymom-=this.ydir/5
                this.stem[this.stem.length-1].xmom = 0
                this.stem[this.stem.length-1].ymom = 0
                this.stem[t].move()
                // this.stem[t].draw()
            ...
            this.stem[this.stem.length-1].draw()
            this.flower.draw()
        }
    }
